...
export default class GameScene extends Phaser.Scene {
    ...
    createGroups() {
        this.enemies = this.physics.add.group({ classType: Enemy, runChildUpdate: true })   // runChildUpdate runs the update function of the object if one exists
    ...
    createPath() {
        ...
        // visualizing path
        // this.graphics.lineStyle(3, 0xffffff, 1) // thickness, color of line, opacity
    ...
    createMap() {
        ...
        this.backgroundLayer = this.bgMap.createStaticLayer('Background', this.tiles, 0, 0) // args are (name of layer from tilemap, tileset used in creation of layer, x position, y position)
        
        // add tower - place we don't want the enemies to go
    ...
    placeTurret(pointer) {
        ...
        if (this.canPlaceTurret(i, j)) {
            ...
            if (!turret) {
                ...
                this.turrets.add(turret)    // add to turet group
            }
            turret.setActive(true)
            turret.setVisible(true)
            turret.place(i, j)

            // logic to update number of turrets
